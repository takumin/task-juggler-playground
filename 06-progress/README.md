...
1. `project` ヘッダの日付は `timezone` を読む前に **UTC で解釈される**。
   `2026-08-03` と書くと開始は `08-03 09:00 JST` になり、
   `booking` で 8/3 0:00 を指すと「プロジェクト期間外」エラーになる。
   この教材では開始を `2026-08-01` に広げて回避している
2. 区間の終了日は 0 時に展開されるので**終了日当日は含まれない**。
   `booking alice 2026-08-03 - 2026-08-08` は 8/3〜8/7 の 5 日間になる
3. `projection` キーワードは**非推奨**。booking があれば projection モードは自動で有効になる
4. booking は本来**エクスポートで機械生成するもの**とされている。
   手書き用に `sloppy` / `overtime` が用意されている
5. `effortdone` / `effortleft` は「未テスト」警告つきの機能。使うなら結果を検証すること
