...
| 2 | 「生成されたレポート」— `out/` のファイルへのリンクと、続けて README の**最初の `## ` の節** |
| 3 | ソース (tjp / tji。すべて折りたたみ、エントリポイントが先頭) |
| 4 | README の2番目の `## ` 以降 |
| 5 | 前後の段階へのナビゲーション (README 末尾の `<hr>` 以降) |

読者は「レポートを開く → 何が出ているかを読む → それを出した tjp を見る → 解説」の
順にたどる。位置 2 はリンクと説明で 1 つの節にまとめてあり、
**README 側の `## レポートの内容` という見出しはページには出ない**
(ページ側の「生成されたレポート」に置き換わる)。
節の中を区切りたいときは `### ` を使う。
...
- 骨格は `## レポートの内容` → `## 学ぶ内容` → (段階固有の解説) →
  `## ハマりどころ` → `## 得られるもの` の4つの見出しで揃える
- `## レポートの内容` は**必ず最初の `## `** にする。この節はレポートのリンクと
  同じ見出しの下に入るので、ここが後ろにあるとリンクとその説明が離れる。中身は
  「`out/` に出るファイルと役割の表」+「結果を数値で示した `### 実行結果`」。
  表はリンク一覧のすぐ下に来るので小見出しを付けない。
  ファイル名は実際の生成物と一致させる (レポート定義を変えたら README も直す)。
  レポートが1つの段階でも表は省かない。
  数値で示せる結果が無い段階 (07 / 08 / 12) は表だけでよい
- `## 学ぶ内容` は**その段階で扱うキーワードの一覧**。
  1 行 1 項目で `- [キーワード][ref] — 説明` の形に統一し、入れ子にしない。
  同種のキーワードは ` / ` で連ねて 1 行にまとめる。
  リンク先は公式マニュアルの該当ページ。注意点はここに書かない。
  挙げてよいのは**その段階の tjp か README 本文で実際に扱っているもの**だけ。
  存在だけ知らせたいものは「(この教材では未使用)」と添える
- 注意事項は `## ハマりどころ` にまとめ、**その段階で理解を誤ると後が詰まる順**に
  番号付きで並べる。太字は 1 項目につき 1 か所まで、誤解の核心となる語句だけに使う
  (項目全体を太字にしない)。
  複数段階に散る話 (レポートの出力先など) はルート README の
  「横断的なハマりどころ」に置き、段階側では繰り返さない。
  その段階でしか出てこない話は、ルートに似た項目があってもここに書く
  (11 の `include` のように、ソースを目の前にした読者がその場で必要とする)
- `## 得られるもの` はその段階を終えて何ができるようになるかを散文で書く
