...
| 3 | README の**最初の `## ` の節** (どの段階も `## レポートの内容`) |
| 4 | ソース (tjp / tji。すべて折りたたみ、エントリポイントが先頭) |
| 5 | README の2番目の `## ` 以降 |
| 6 | 前後の段階へのナビゲーション (README 末尾の `<hr>` 以降) |

読者は「レポートを開く → 何が出ているかを読む → それを出した tjp を見る → 解説」の
順にたどる。最初の `## ` の節だけがレポートのリンク一覧の直後に回されるので、
そこには**そのリンクを開いた読者がすぐ必要とすること**だけを書く。
...
- `## レポートの内容` は**必ず最初の `## `** にする。生成物一覧の直後に回されるため、
  ここが後ろにあるとリンクとその説明が離れる。中身は
  「`out/` に出るファイルと役割の表」+「結果を数値で示した表」の2つ。
